...
from sgl_jax.srt.kernels.causal_conv1d import strided_ldst
...
class BufferWrapper(ABC):
    ...
    def is_upper_oob(self, row) -> jax.Array:
        num_tiles = self.metadata_ref.num_tiles[0]
...
class XBuffer(BufferWrapper):
    def copy_in(self, b_start, slot, sem):
        ...
        @pl.when(jnp.logical_not(is_no_op))
        def _copy():
    ...
    def wait_in(self, b_start, slot, sem):
        ...
        @pl.when(jnp.logical_not(is_no_op))
        def _wait():
            slot_ref = self.get_slot_vmem(slot)
            pltpu.make_async_copy(slot_ref, slot_ref, sem).wait()
...
class ConvStateBuffer(BufferWrapper):
    def copy_in(self, b_start, slot, sem):
        ...
        for idx in range(self.cfgs.tile_size):
            ...
            for copy_size in range(1, self.cfgs.kernel_size):
                should_copy = jnp.logical_and(jnp.logical_not(is_no_op), sz_from_old == copy_size)

                @pl.when(should_copy)
                def _copy(copy_size=copy_size, idx=idx, state_idx=state_idx):
                    start_from_old = self.cfgs.prev_kernel_size - copy_size
                    pltpu.make_async_copy(
                        self.hbm_ref.at[state_idx, pl.ds(start_from_old, copy_size)],
                        self.get_slot_vmem(slot).at[idx, pl.ds(0, copy_size)],
                        sem,
                    ).start()
    ...
    def wait_in(self, b_start, slot, sem):
        for idx in range(self.cfgs.tile_size):
            ...
            for copy_size in range(1, self.cfgs.kernel_size):
                ...
                @pl.when(sz_from_old == copy_size)
                def _wait(copy_size=copy_size, idx=idx):
                    row_ref = self.get_slot_vmem(slot).at[idx, pl.ds(0, copy_size)]
    ...
    def copy_out(self, b_start, slot, sem):
        for idx in range(self.cfgs.tile_size):
            ...
            def _copy(idx=idx, state_idx=state_idx):
                pltpu.make_async_copy(
                    self.get_slot_vmem(slot).at[pl.ds(idx, 1)],
                    self.hbm_ref.at[pl.ds(state_idx, 1)],
                    sem,
                ).start()
    ...
    def wait_out(self, b_start, slot, sem):
        ...
        for idx in range(self.cfgs.tile_size):
            b_idx = b_start + idx
            should_write = self.metadata_ref.b_idx_should_write[b_idx]
            should_wait = jnp.logical_and(jnp.logical_not(is_no_op), should_write != 0)

            @pl.when(should_wait)
            def _wait(idx=idx):
                row_ref = self.get_slot_vmem(slot).at[pl.ds(idx, 1)]
                pltpu.make_async_copy(row_ref, row_ref, sem).wait()
...
def main_kernel(
    ...
    *,
    ...
    num_tiles = metadata_ref.num_tiles[0]
...
def preprocess_metadata(
    ...
    b_idx_to_s_idx = jnp.repeat(seqs, query_lens, total_repeat_length=cfgs.batch_size)
    ...
    b_idx_to_sz_from_old = jnp.minimum(b_idx_to_sz_from_old, cfgs.prev_kernel_size)
    ...
    return MetadataRef(
        num_tiles=jnp.expand_dims(pl.cdiv(num_tokens, cfgs.tile_size), axis=0),
...
def ragged_causal_conv1d(
    ...
    *,
    ...
    out, new_conv_state = pl.pallas_call(
        ...
        input_output_aliases={len(metadata): 0, len(metadata) + 1: 1},
