...
def _assert_fused_moe_v2_supported(moe_backend: MoEBackend, architectures: list[str]) -> None:
    ...
    assert any(arch in _FUSED_MOE_V2_SUPPORTED_ARCHITECTURES for arch in architectures), (
        "moe_backend='fused_v2' only supports Bailing/MiMo/Inkling model architectures for now; "
...
class ModelConfig:
    ...
    def get_total_num_kv_heads_with_replication(self, tensor_parallel_size: int) -> int:
        ...
        total_num_kv_heads = self.get_total_num_kv_heads()
        replicated_heads = max(total_num_kv_heads, tensor_parallel_size)
        swa_heads = getattr(
            self,
            "_original_swa_num_key_value_heads",
            getattr(self.hf_text_config, "swa_num_key_value_heads", total_num_kv_heads),
        )
        replicated_swa_heads = max(swa_heads, tensor_parallel_size)
        return max(replicated_heads, replicated_swa_heads)
