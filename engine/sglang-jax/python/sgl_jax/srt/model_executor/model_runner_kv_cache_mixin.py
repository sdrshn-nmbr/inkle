...
def _compute_recurrent_per_req_bytes(
    ...
    if has_temporal_state:
        per_req_recurrent = (
            ...
            * head_dim
            * head_dim
            * temporal_dtype_bytes
        )
    conv_channels = (
        num_layers * proj_size
        if conv_channel_sizes is None
        else sum(sum(sizes) for sizes in conv_channel_sizes)
    )
    assert conv_channels % tp_size == 0, (
        f"total conv channels {conv_channels} must be divisible by tp_size {tp_size}"
    )
    per_req_conv = (
        (conv_kernel_size - 1)
        * (conv_channels // tp_size)
        * conv_dtype_bytes
    )
...
def _per_req_state_bytes_from_config(cfg, tp_size: int) -> int:
    ...
    return _compute_recurrent_per_req_bytes(
        ...
        conv_channel_sizes=state_params.conv_channel_sizes,
        has_temporal_state=state_params.has_temporal_state,
    )
...
class ModelRunnerKVCacheMixin:
    ...
    @property
    def inkling_config(self: ModelRunner):
        if self.model_config.hf_config.model_type != "inkling_mm_model":
            return None
    ...
    def linear_recurrent_config(self: ModelRunner):
        ...
        if self.inkling_config is not None:
            return self.inkling_config
